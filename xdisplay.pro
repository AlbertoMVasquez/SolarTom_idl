...
;  if not keyword_set(rad_range)   then rad_range   = [1.02,1.23]
  ...
  if keyword_set(rad_range) then  begin
     ...
     xhisto,map=map,nr=nr,nt=nt,np=np,rmin=rmin,rmax=rmax,rad_range=rad_range,lat_range=lat_range,win=win,dir=dir,file=file,titulo='Histogram of '+titulo+strmid(string(rad_range[0]),6,5)+'-'+strmid(string(rad_range[1]),6,5),/eps,sufijo=sufijo
  endif
