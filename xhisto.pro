pro xhisto,map=map,nr=nr,nt=nt,np=np,radii=radii,rad_range=rad_range,lat_range=lat_range,lon_range=lon_range,win=win,titulo=titulo,dir=dir,file=file,sufijo=sufijo,mini=mini,maxi=maxi
  ...
  if (where(x_data lt 0.))(0) eq -1 then Nneg = 0.
  if (where(x_data eq 0.))(0) eq -1 then Nnul = 0.
  Ntot=Npos+Nnul+Nneg
  ...
  histo_x_data = histogram(x_data,binsize=maxdata/Nbins,locations=xval) / total(x_data)
   ...
   ; calculo la mediana solo con los valores positivos (excluyo ZDAs)
   ind = where(x_data gt 0.)
   ...
   ;med = median(x_data     ) 
     ...
     xyouts,0.6*[1],top-0.05*[4],['Median value:'+string(med)],/normal,Font=1,charsize=1.5
